...
(vanish bin:bit_1 (begin (if (- bin:IS_BYTE 1) (begin (if bin:LOW_4 (- bin:BIT_1 1) (if (- bin:COUNTER 0) bin:BIT_1 (if (- bin:COUNTER bin:LOW_4) (- bin:BIT_1 (+ (shift bin:BIT_1 -1) 1)) (- bin:BIT_1 (shift bin:BIT_1 -1))))))) (if (- bin:IS_SIGNEXTEND 1) (begin (if (- 15 bin:LOW_4) (- bin:BIT_1 1) (if (- bin:COUNTER 0) bin:BIT_1 (if (- bin:COUNTER (- 15 bin:LOW_4)) (- bin:BIT_1 (+ (shift bin:BIT_1 -1) 1)) (- bin:BIT_1 (shift bin:BIT_1 -1)))))))))

(vanish bin:binary_constraints (begin (* bin:IS_AND (- 1 bin:IS_AND)) (* bin:IS_OR (- 1 bin:IS_OR)) (* bin:IS_XOR (- 1 bin:IS_XOR)) (* bin:IS_NOT (- 1 bin:IS_NOT)) (* bin:IS_BYTE (- 1 bin:IS_BYTE)) (* bin:IS_SIGNEXTEND (- 1 bin:IS_SIGNEXTEND)) (* bin:SMALL (- 1 bin:SMALL)) (* bin:BITS (- 1 bin:BITS)) (* bin:NEG (- 1 bin:NEG)) (* bin:BIT_B_4 (- 1 bin:BIT_B_4)) (* bin:BIT_1 (- 1 bin:BIT_1))))
...
(vanish bin:is-signextend-result (ifnot bin:IS_SIGNEXTEND (if (- bin:ONE_LINE_INSTRUCTION 1) (begin (- bin:RESULT_HI bin:ARGUMENT_2_HI) (- bin:RESULT_LO bin:ARGUMENT_2_LO)) (if bin:SMALL (begin (- bin:RESULT_HI bin:ARGUMENT_2_HI) (- bin:RESULT_LO bin:ARGUMENT_2_LO)) (begin (if bin:BIT_B_4 (begin (- bin:BYTE_5 (* bin:NEG 255)) (if bin:BIT_1 (- bin:BYTE_6 (* bin:NEG 255)) (- bin:BYTE_6 bin:BYTE_4))) (begin (if bin:BIT_1 (- bin:BYTE_5 (* bin:NEG 255)) (- bin:BYTE_5 bin:BYTE_3)) (- bin:RESULT_LO bin:ARGUMENT_2_LO))))))))

(vanish bin:small (if (- bin:COUNTER 15) (if bin:ARGUMENT_1_HI (if (- bin:ARGUMENT_1_LO (+ (* 16 (shift bin:BITS -4)) (* 8 (shift bin:BITS -3)) (* 4 (shift bin:BITS -2)) (* 2 (shift bin:BITS -1)) bin:BITS)) (- bin:SMALL 1) bin:SMALL))))
...
(vanish bin:target-constraints (if (- bin:COUNTER 15) (begin (- bin:ACC_1 bin:ARGUMENT_1_HI) (- bin:ACC_2 bin:ARGUMENT_1_LO) (- bin:ACC_3 bin:ARGUMENT_2_HI) (- bin:ACC_4 bin:ARGUMENT_2_LO) (- bin:ACC_5 bin:RESULT_HI) (- bin:ACC_6 bin:RESULT_LO))))

(vanish bin:mli-incrementation (ifnot bin:MLI (if (- bin:COUNTER 15) (- (shift bin:STAMP 1) (+ bin:STAMP 1)) (- (shift bin:COUNTER 1) (+ bin:COUNTER 1)))))

(vanish bin:stamp-increments (* (- (shift bin:STAMP 1) (+ bin:STAMP 0)) (- (shift bin:STAMP 1) (+ bin:STAMP 1))))

(vanish bin:is-byte-result (ifnot bin:IS_BYTE (if (- bin:ONE_LINE_INSTRUCTION 1) (begin bin:RESULT_HI bin:RESULT_LO) (begin bin:RESULT_HI (- bin:RESULT_LO (* bin:SMALL bin:PIVOT))))))
...
(vanish bin:set-oli-mli (if (+ bin:IS_BYTE bin:IS_SIGNEXTEND) bin:ONE_LINE_INSTRUCTION (if bin:ARGUMENT_1_HI bin:ONE_LINE_INSTRUCTION (- bin:ONE_LINE_INSTRUCTION 1))))
...
(vanish bin:oli-incrementation (ifnot bin:ONE_LINE_INSTRUCTION (- (shift bin:STAMP 1) (+ bin:STAMP 1))))

(vanish:last bin:last-row (if (- bin:MLI 1) (- bin:COUNTER 15)))

(vanish bin:oli-mli-exclusivity (- (+ bin:ONE_LINE_INSTRUCTION bin:MLI) (+ bin:IS_AND bin:IS_OR bin:IS_XOR bin:IS_NOT bin:IS_BYTE bin:IS_SIGNEXTEND)))

(vanish bin:countereset (ifnot (- (shift bin:STAMP 1) bin:STAMP) (shift bin:COUNTER 1)))
