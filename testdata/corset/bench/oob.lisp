...
(defun (jumpi---jump-not-attempted)         (i1 [DATA 6]))
(defun (jumpi---guaranteed-exception)       [DATA 7])
(defun (jumpi---jump-must-be-attempted)     (i1 [DATA 8]))
(defun (jumpi---valid-pc-new)               (i1 OUTGOING_RES_LO))
(defun (jumpi---jump-cond-is-zero)          (i1 (next OUTGOING_RES_LO)))
...
(defun ((rdc---rdc-roob)) (- 1 (i1 OUTGOING_RES_LO)))
...
(defun (create---insufficient-balance-abort)           (i1 OUTGOING_RES_LO))
(defun (create---stack-depth-abort)                    (- 1 (i1 (next OUTGOING_RES_LO))))
(defun (create---nonzero-nonce)                        (- 1 (i1 (shift OUTGOING_RES_LO 2))))
(defun (create---creator-nonce-abort)                  (- 1 (i1 (shift OUTGOING_RES_LO 3))))
...
(defun (prc---hub-success)                    (i1 [DATA 4]))
(defun (prc---ram-success)                    (i1 [DATA 4]))
(defun (prc---return-gas)                     [DATA 5])
(defun (prc---extract-call-data)              (i1 [DATA 6]))
(defun (prc---empty-call-data)                (i1 [DATA 7]))
(defun (prc---r@c-nonzero)                    (i1 [DATA 8]))
(defun (prc---cds-is-zero)                    (i1 OUTGOING_RES_LO))
(defun (prc---r@c-is-zero)                    (i1 (next OUTGOING_RES_LO)))
...
(defun (prc-ecpairing---is-multiple_192)           (i1 (shift OUTGOING_RES_LO 3)))
...
(defconstraint prc-ecpairing---justify-hub-predictions (:guard (* (assumption---fresh-new-stamp) (prc-ecpairing---standard-precondition)))
  (if (== 0 (prc---hub-success))
      (begin
       (∨ (== 0 (prc-ecpairing---is-multiple_192)) (!= 0 (prc-ecpairing---insufficient-gas)))
       (vanishes! (prc---return-gas))
...
(defun (prc-modexp-xbs---compute-max)              (i1 [DATA 4]))
(defun (prc-modexp-xbs---max-xbs-ybs)              [DATA 7])
(defun (prc-modexp-xbs---xbs-nonzero)              (i1 [DATA 8]))
...
(defun (prc-modexp-lead---load-lead)                           (i1 [DATA 4]))
(defun (prc-modexp-lead---cds-cutoff)                          [DATA 6])
(defun (prc-modexp-lead---ebs-cutoff)                          [DATA 7])
(defun (prc-modexp-lead---sub-ebs_32)                          [DATA 8])
(defun (prc-modexp-lead---ebs-is-zero)                         (i1 OUTGOING_RES_LO))
(defun (prc-modexp-lead---ebs-less-than_32)                    (i1 (next OUTGOING_RES_LO)))
(defun (prc-modexp-lead---call-data-contains-exponent-bytes)   (i1 (shift OUTGOING_RES_LO 2)))
