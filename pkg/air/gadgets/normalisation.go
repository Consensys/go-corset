...
func ApplyPseudoInverseGadget(e air.Expr, schema *air.Schema) air.Expr {
	...
	if !ok {
		// Add computed column
		index = schema.AddAssignment(assignment.NewComputedColumn(ctx.Module, name, ctx.Multiplier, ie))
	}
	...
	schema.AddVanishingConstraint(l_name, ctx.Module, ctx.Multiplier, nil, e_implies_one_e_e)
	// Ensure (e/e != 0) ==> (1 == e/e)
	r_name := fmt.Sprintf("[%s =>]", ie.String())
	schema.AddVanishingConstraint(r_name, ctx.Module, ctx.Multiplier, nil, inv_e_implies_one_e_e)
	// Done
...
// expression.
func (e *Inverse) Context(schema sc.Schema) (sc.EvalContext, bool) {
