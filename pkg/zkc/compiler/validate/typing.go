...
func (p *TypeChecker) typeAssignment(s *stmt.Assign[symbol.Resolved], env variable.Map) []source.SyntaxError {
	var (
		...
		rhs, errs = p.typeExpression(s.Source, env)
	)
	// check
	if len(errs) != 0 {
		return errs
...
func (p *TypeChecker) typeExpression(e ast.Expr, env variable.Map) (typing.Type, []source.SyntaxError) {
	...
	case *expr.NonLocalAccess[symbol.Resolved]:
		return p.typeNonLocalAccess(e, env)
...
func (p *TypeChecker) typeNonLocalAccess(e *expr.NonLocalAccess[symbol.Resolved], env variable.Map,
) (typing.Type, []source.SyntaxError) {
	//
	var (
		bound    = big.NewInt(2)
		nonLocal = p.lookup(e.Name)
	)
	//
	if t, ok := nonLocal.(*ast.Constant); ok {
		bitwidth := t.DataType.BitWidth()
		// compute 2^bitwidth
		bound.Exp(bound, big.NewInt(int64(bitwidth)), nil)
		// Subtract 1 because interval is inclusive.
		bound.Sub(bound, big.NewInt(1))
	...
	//
	return nil, p.srcmaps.SyntaxErrors(e, "expected uint")
}
