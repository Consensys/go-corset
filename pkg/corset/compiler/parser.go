...
func ParseSourceFiles(files []*source.File, strictMode bool) (ast.Circuit,
	*source.Maps[ast.Node], []SyntaxError) {
	...
	for _, file := range files {
		c, srcmap, errs := ParseSourceFile(file, strictMode)
		// Handle errors
		if len(errs) > 0 {
...
func ParseSourceFile(srcfile *source.File, strictMode bool) (ast.Circuit,
	*source.Map[ast.Node], []SyntaxError) {
	...
	p := NewParser(srcfile, srcmap, strictMode)
...
func (p *Parser) parseDefAlias(functions bool, elements []sexp.SExp) (ast.Declaration, []SyntaxError) {
	var (
		errors  []SyntaxError
	...
	for i := 1; i < len(elements); i += 2 {
		...
		if i+1 == len(elements) {
			...
			name := ast.NewUnboundName[ast.Binding](path, functions)
			//
	...
	// Done
	return ast.NewDefAliases(functions, aliases, names), errors
}
		...
		for i := 0; i < sexpSources.Len(); i++ {
			...
			if symbol := sexpSources.Get(i).AsSymbol(); symbol == nil {
				...
				if err != nil {
					...
					// Valid symbol
					sources[i] = ast.NewVariableAccess(path, i == 0, nil)
					// Update source mapping
...
func (p *Parser) parseDefInterleavedSourceColumn(source *sexp.Symbol) (ast.TypedSymbol, []SyntaxError) {
	if path, err := parseQualifiableName(source.Value); err != nil {
		...
		varAccess := ast.NewVariableAccess(path, false, nil)
...
func (p *Parser) parseDefLookup(elements []sexp.SExp) (ast.Declaration, []SyntaxError) {
	// Extract items
	handle := elements[1]
	target, tgtErrors := p.parseDefLookupVector("target", nil, elements[2])
	source, srcErrors := p.parseDefLookupVector("source", nil, elements[3])
	...
	if source.Len() != target.Len() {
		msg := fmt.Sprintf("differing number of source and target columns (%d v %d)", source.Len(), target.Len())
...
func (p *Parser) parseDefConditionalLookup(elements []sexp.SExp) (ast.Declaration, []SyntaxError) {
	...
	var (
		handle         = elements[1]
		targetsSExp    sexp.SExp
		target, source ast.LookupVector
		errs1, errs2   []SyntaxError
	)
	//
	if len(elements) == 6 {
		// Source and target selector
		targetsSExp = elements[3]
		target, errs1 = p.parseDefLookupVector("target", elements[2], elements[3])
		source, errs2 = p.parseDefLookupVector("source", elements[4], elements[5])
	} else {
		// Source selector only
		targetsSExp = elements[2]
		target, errs1 = p.parseDefLookupVector("target", nil, elements[2])
		source, errs2 = p.parseDefLookupVector("source", elements[3], elements[4])
	}
	// Combine errors
	...
	if source.Len() != target.Len() {
		msg := fmt.Sprintf("differing number of source and target columns (%d v %d)", source.Len(), target.Len())
		errors = append(errors, *p.translator.SyntaxError(targetsSExp, msg))
	}
	// Error check
	if len(errors) != 0 {
		return nil, errors
...
func (p *Parser) parseDefLookupVector(handle string, sexpSelector sexp.SExp,
	element sexp.SExp) (ast.LookupVector, []SyntaxError) {
	//
	var (
		sexpTerms = element.AsList()
		errors    []SyntaxError
		terms     []ast.Expr
		selector  ast.Expr
	)
	...
	if sexpTerms == nil {
		...
		return ast.LookupVector{}, p.translator.SyntaxErrors(element, msg)
	}
	// parse selector (if applicable)
	if sexpSelector != nil {
		selector, errors = p.translator.Translate(sexpSelector)
	}
	// Parse terms
	if len(errors) == 0 {
		terms = make([]ast.Expr, sexpTerms.Len())
		//
		for i := 0; i != sexpTerms.Len(); i++ {
			var errs []SyntaxError
			// Translate source expressions
			terms[i], errs = p.translator.Translate(sexpTerms.Get(i))
			errors = append(errors, errs...)
		}
	}
	// Done
	return ast.LookupVector{Selector: selector, Terms: terms}, errors
}
...
func (p *Parser) parsePermutedColumnAccess(e sexp.SExp) (*ast.VariableAccess, *bool, *SyntaxError) {
	...
	if path, err := parseQualifiableName(name); err == nil {
		colAccess := ast.NewVariableAccess(path, false, nil)
		// Update source mapping
...
func (p *Parser) parseFunctionParameter(element sexp.SExp) (*ast.DefParameter, []SyntaxError) {
	...
	if isIdentifier(element) {
		return ast.NewDefParameter(element.AsSymbol().Value, ast.NewFieldType()), nil
...
func (p *Parser) parseConstraintAttributes(module util.Path, attributes sexp.SExp) (domain util.Option[int],
	...
	for i := 0; i < attrs.Len(); i++ {
		...
		if ith.AsSymbol() == nil {
			...
			case ":perspective":
				i++
				perspective, errs = parseSymbolName[*ast.PerspectiveBinding](p, attrs.Get(i), module, false, nil)
			default:
...
func parseSymbolName[T ast.Binding](p *Parser, symbol sexp.SExp, module util.Path, function bool,
	...
	name := ast.NewBoundName[T](*path, function, binding)
	// Update source mapping
...
func varAccessParserRule(col string) (ast.Expr, bool, error) {
	...
	//
	return ast.NewVariableAccess(path, false, nil), true, nil
}
...
func invokeParserRule(p *Parser) sexp.ListRule[ast.Expr] {
	return func(list *sexp.List) (ast.Expr, []SyntaxError) {
		...
		// Done
		return &ast.Invoke{Name: varaccess, Signature: nil, Args: args}, nil
	}
}
