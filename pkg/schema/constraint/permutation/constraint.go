...
// to debugging.
func (p Constraint[F]) Consistent(schema schema.AnySchema[F, schema.State]) []error {
...
// target columns.
func (p Constraint[F]) Accepts(tr trace.Trace[F], _ schema.AnySchema[F, schema.State]) (bit.Set, schema.Failure) {
	var (
...
func (p Constraint[F]) Lisp(mapping schema.AnySchema[F, schema.State]) sexp.SExp {
	var (
