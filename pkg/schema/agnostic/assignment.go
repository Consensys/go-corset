...
// SignBit determines whether or not the given assignment has a dedicated
// sign bit already and, if so, returns its ID.
func (p *Assignment) SignBit(env sc.RegisterMapping) (sc.RegisterId, bool) {
	var (
		n = len(p.LeftHandSide) - 1
	)
	//
	if n < 0 {
	...
	//
	lastReg := p.LeftHandSide[n]
	// Look for single sign bit
	return lastReg, env.Limb(lastReg).Width == 1
}
...
func (p *Assignment) Overflow(env sc.RegisterMapping) (uint, uint, bool) {
	var (
		// Determine lhs width
		lhs = CombinedWidthOfLimbs(env, p.LeftHandSide...)
		// Determine rhs width
...
func (p *Assignment) ResolveOverflow(env sc.RegisterAllocator) (Assignment, Polynomial) {
	var overflow, underflow, signed = p.Overflow(env)
	// Check whether anything to resolve
	if overflow == 0 && underflow == 0 && !signed {
		// No, nothing to resolve.
		return *p, nil
	} else if overflow == 0 {
		return p.resolveUnsignedUnderflow(underflow, env)
	} else if underflow == 0 && !signed {
...
func (p *Assignment) resolveUnsignedUnderflow(underflow uint, env sc.RegisterAllocator) (Assignment, Polynomial) {
	var (
		...
		borrow              []Monomial
		lhs                 = p.LeftHandSide
		signBit, hasSignBit = p.SignBit(env)
	)

	// Add carry component (if applicable)
	if underflow == 0 && !hasSignBit {
		panic("todo")
	} else if underflow != 0 || !hasSignBit {
		// Allocated signed bit
		signBit = env.AllocateCarry("s", 1)
		// Reduce number of required underflow bits.
		underflow--
		// Append sign bit to borrow calculation
		borrow = append(borrow, poly.NewMonomial(*math.NegPow2(underflow), signBit))
		//
		if underflow > 0 {
			// Allocate new register to get an Id
			borrowRegId := env.AllocateCarry("b", underflow)
			// Construct carry to be propagated forward
			borrow = append(borrow, poly.NewMonomial(one, signBit, borrowRegId))
			// Update left-hand side to include carry
			lhs = array.Append(lhs, borrowRegId)
		}
	}
	// Include new signbit (if applicable)
	if !hasSignBit {
		// Update left-hand side to include sign bit
		lhs = array.Append(lhs, signBit)
	}
	// Resolved
	return NewAssignment(lhs, p.RightHandSide), tmp.Set(borrow...)
}
...
func coalesceAssignments(assignments []Assignment, bandwidth uint, env sc.RegisterAllocator) []Assignment {
	...
	for i, a := range assignments {
		...
		// required width.
		if nWidth > bandwidth {
			next, carry = coalesce(assignments[start:i], carry, env)
	...
	// Coalesce final group
	next, carry = coalesce(assignments[start:], carry, env)
	coalesced = append(coalesced, next)
	// Sanity check carry was allocated.
	if carry != nil && carry.Len() != 0 {
