...
func (p *Assignment) Width(env sc.RegisterMap) uint {
	var (
		// Determine lhs width
...
func (p *Assignment) Overflow(env sc.RegisterMap) (uint, uint) {
	var (
		// Determine lhs width
		lhs = CombinedWidthOfRegisters(env, p.LeftHandSide...)
		// Determine rhs width
...
func (p *Assignment) Terminate(env sc.RegisterMap) (Assignment, StaticPolynomial) {
	var (
	...
	if lhs < rhs {
		panic(fmt.Sprintf("unaccounted for carry bits (%dbits := %dbits)", lhs, rhs))
	}
	// Return without change
	return *p, tmp.Set()
}

// Link attempts to resolve, for a non-terminating internal assignment, any
// imbalance between the number of bits produced by the right-hand side of an
...
func (p *Assignment) Link(env sc.RegisterAllocator) (Assignment, StaticPolynomial) {
	...
	if p.RightHandSide.Signed() {
		return p.linkSigned(max(overflow, underflow), env)
	} else if underflow > 0 {
		// Unsigned assignments should not have underflow!
		panic("unreachable")
	}
	//
...
func (p *Assignment) linkSigned(underflow uint, env sc.RegisterAllocator) (Assignment, StaticPolynomial) {
	...
	if underflow > 0 {
		...
		// Update left-hand side to include carry
		lhs = array.Append(lhs, carryRegId)
	}
	// Update left-hand side to include sign bit
	lhs = array.Append(lhs, signBit)
	// Resolved
	return NewAssignment(lhs, p.RightHandSide), tmp.Set(carry...)
}
...
func (p *Assignment) initialiseSplit(env sc.RegisterMap) []Assignment {
	var (
...
func identifyEnclosingRegister(regs []sc.RegisterId, value big.Int, env sc.RegisterMap) (uint, uint) {
	var bitOffset uint
	//
	for i, lid := range regs {
		var reg = env.Register(lid)
		// Value contained by this register?
		if withinBitRange(value, bitOffset, bitOffset+reg.Width) {
			// Yes!
...
func coalesceAssignments(assignments []Assignment, bandwidth uint, env sc.RegisterAllocator) []Assignment {
	...
	// checked in Terminate.
	next, _ = coalesce(true, assignments[start:], carry, env)
	coalesced = append(coalesced, next)
	//
	return coalesced
}
...
func coalesce(last bool, assignments []Assignment, carry StaticPolynomial,
	...
	for i, ith := range assignments {
		...
		// update bit offset
		offset += CombinedWidthOfRegisters(env, ith.LeftHandSide...)
	}
	// Construct new assignment
	assign := NewAssignment(lhs, rhs)
	//
	if last {
		return assign.Terminate(env)
	}
...
// nolint
func debugAssignments(env sc.RegisterAllocator, assignments ...Assignment) {
	for i, ith := range assignments {
