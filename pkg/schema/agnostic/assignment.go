...
func (p *Assignment) String(env sc.RegisterMap) string {
	var (
...
func (p *Assignment) Width(env sc.RegisterMap) (bitwidth uint, signed bool) {
	var (
		// Determine lhs width
	...
	// Determine rhs width
	rhs, signed = WidthOfPolynomial(p.RightHandSide, env.Registers())
	//
...
func (p *Assignment) Signed(env sc.RegisterMap) bool {
...
// registers.
func (p *Assignment) Split(field sc.FieldConfig, env sc.RegisterAllocator) (eqs []Assignment) {
	var width, _ = p.Width(env)
	// Check whether any splitting required
	if width > field.FieldBandWidth {
		// Yes!
...
func (p *Assignment) chunkUp(field sc.FieldConfig, mapping sc.RegisterAllocator) []Assignment {
	var (
		last      Polynomial
		iteration = 0
...
func initialiseLhsChunks(regs []sc.RegisterId, field sc.FieldConfig, signed bool, mapping sc.RegisterMap) []LhsChunk {
	var (
		chunks     []Chunk[[]sc.RegisterId]
		chunkWidth = determineInitialChunkWidth(field)
	)
	//
	for len(regs) != 0 {
		var chunk Chunk[[]sc.RegisterId]
...
func determineInitialChunkWidth(field sc.FieldConfig) uint {
...
func getNextLhsChunk(regs []sc.RegisterId, chunkWidth uint, signed bool,
	mapping sc.RegisterMap) (LhsChunk, []sc.RegisterId) {
	//
	var bitwidth uint
	//
	for i, r := range regs {
...
func determineRhsChunks(p Polynomial, chunks []LhsChunk, field sc.FieldConfig,
	mapping sc.RegisterAllocator) ([]LhsChunk, []RhsChunk, bit.Set) {
	//
	var (
		rhsChunks []RhsChunk
		lhsChunks []LhsChunk
	...
	for i, chunk := range chunks {
		...
		chunkWidth, s := RawWidthOfPolynomial(remainder, mapping.Registers())
		...
		if chunkWidth > field.FieldBandWidth {
			// No, it does not.
...
func propagateCarry(chunk LhsChunk, overflow uint, carry Polynomial,
	mapping sc.RegisterAllocator) (Polynomial, LhsChunk) {
	// Overflow case.
	var tmp Polynomial
	// Allocate new register to get an Id
	carryRegId := mapping.AllocateCarry("c", overflow)
...
func propagateBorrow(chunk LhsChunk, overflow uint, carry Polynomial,
	mapping sc.RegisterAllocator) (Polynomial, LhsChunk) {
	var borrow Polynomial
	// Allocate new register to get an Id
	signBit := mapping.AllocateCarry("b", 1)
...
// assignment.
type LhsChunk = Chunk[[]sc.RegisterId]

// RhsChunk captures the chunk type used for the Right-Hand Side (RHS) of an
// assignment.
type RhsChunk = Chunk[Polynomial]

// StaticPoly2String provides a convenient helper function for debugging polynomials.
func StaticPoly2String(p Polynomial, env sc.RegisterMap) string {
	return poly.String(p, func(r sc.RegisterId) string {
		return env.Register(r).Name
	})
}
...
type RegisterSplitter struct {
	...
	parents     map[sc.RegisterId]sc.RegisterId
}
...
func NewRegisterSplitter(n uint) RegisterSplitter {
	var (
		...
		parents   = make(map[sc.RegisterId]sc.RegisterId)
	)
	//
...
func (p *RegisterSplitter) ParentOf(v sc.RegisterId) sc.RegisterId {
...
func (p *RegisterSplitter) Reset() {
	...
	p.parents = make(map[sc.RegisterId]sc.RegisterId)
}
...
func (p *RegisterSplitter) Apply(poly Polynomial, mapping sc.RegisterAllocator) Polynomial {
	var cache = make(map[sc.RegisterId]Polynomial)
	//
	...
	//
	return SubstitutePolynomial(poly, func(reg sc.RegisterId) Polynomial {
...
func (p *RegisterSplitter) splitVariable(rid sc.RegisterId, div uint, mapping sc.RegisterAllocator) (r Polynomial) {
	var (
	...
	limbWidths := LimbWidths(maxWidth, reg.Width)
	// Allocate limbs
...
func (p *RegisterSplitter) String(mapping sc.RegisterMap) string {
	var (
		builder strings.Builder
		first   = true
	)
...
func (p *RegisterSplitter) alloc(parent sc.RegisterId, limbWidths []uint,
	mapping sc.RegisterAllocator) []sc.RegisterId {
	//
	var (
		reg   = mapping.Register(parent)
		limbs = mapping.AllocateCarryN(reg.Name, limbWidths)
	)
...
//
// nolint
func debugChunks(lhs []LhsChunk, rhs []RhsChunk, mapping sc.RegisterMap) {
	//
	for i := len(lhs); i > 0; i-- {
		ith := lhs[i-1]
