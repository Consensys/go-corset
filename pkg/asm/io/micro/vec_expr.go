...
type VecExpr struct {
	util.Union[agnostic.Vector, big.Int]
}
...
func NewVecExpr(e Expr) VecExpr {
	//
	if e.HasFirst() {
		regs := agnostic.NewVector(e.First())
		return VecExpr{util.Union1[agnostic.Vector, big.Int](regs)}
	}
	//
	return VecExpr{util.Union2[agnostic.Vector](e.Second())}
}

// Bitwidth returns the minimum number of bits required to store any evaluation
// of this expression.
func (e VecExpr) Bitwidth(fn schema.RegisterMap) uint {
	if e.HasFirst() {
...
func (e VecExpr) String(fn schema.RegisterMap) string {
	if e.HasFirst() {
...
func (e VecExpr) Split(mapping schema.RegisterLimbsMap) VecExpr {
	if e.HasSecond() {
		return e
	}
	//
	return VecExpr{util.Union1[agnostic.Vector, big.Int](e.First().Split(mapping))}
}
