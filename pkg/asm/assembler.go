...
// Assemble accepts a given source file representing an assembly language
// program, and assembles it into an instruction sequence which can then the
// executed.
func Assemble(srcfile *source.File) ([]Function, []source.SyntaxError) {
	assembler := NewAssembler(srcfile)
	// Parse functions
...
// Assembler is a parser for assembly language.
...
// NewAssembler constructs a new assembler for a given source file.
func NewAssembler(srcfile *source.File) *Assembler {
	return &Assembler{srcfile, nil, 0}
}

func (p *Assembler) parse() ([]Function, []source.SyntaxError) {
	var fns []Function
...
func (p *Assembler) lex() []source.SyntaxError {
	var (
...
func (p *Assembler) parseFunction() (Function, []source.SyntaxError) {
	var (
		fn              Function
		env             Environment
		insn            Instruction
	...
	// Parse inputs
	if inputs, errs = p.parseArgsList(INPUT_REGISTER); len(errs) > 0 {
		return fn, errs
	}
	// Parse '->'
	...
	// Parse outputs
	if outputs, errs = p.parseArgsList(OUTPUT_REGISTER); len(errs) > 0 {
		return fn, errs
	}
...
func (p *Assembler) parseArgsList(kind uint8) ([]Register, []source.SyntaxError) {
	var (
		arg   string
		width uint
		errs  []source.SyntaxError
...
func (p *Assembler) parseType() (uint, []source.SyntaxError) {
	var (
...
func (p *Assembler) parseInstruction(pc uint, env *Environment) (Instruction, []source.SyntaxError) {
	var (
...
func (p *Assembler) parseGoto(env *Environment) (Instruction, []source.SyntaxError) {
...
func (p *Assembler) parseLabel(pc uint, env *Environment) (Instruction, []source.SyntaxError) {
...
func (p *Assembler) parseVar(env *Environment) (Instruction, []source.SyntaxError) {
	var (
		errs  []source.SyntaxError
		name  string
		width uint
...
func (p *Assembler) parseIf(env *Environment, sign bool) (Instruction, []source.SyntaxError) {
	var (
		errs     []source.SyntaxError
		register string
		label    string
...
func (p *Assembler) parseAssignment(env *Environment) (Instruction, []source.SyntaxError) {
	var (
		lhs      []uint
		rhs      []uint
		constant big.Int
...
func (p *Assembler) parseAssignmentLhs(env *Environment) ([]uint, []source.SyntaxError) {
	var (
...
func (p *Assembler) parseAssignmentRhs(env *Environment) (uint, []uint, big.Int, []source.SyntaxError) {
	var (
		constant big.Int
		rhs      []uint
		kind     uint = ADD
...
func (p *Assembler) parseAssignmentOp() (lex.Token, bool) {
	lookahead := p.lookahead()
	//
...
func (p *Assembler) parseRegister(env *Environment) (uint, []source.SyntaxError) {
	lookahead := p.lookahead()
...
func (p *Assembler) parseKeyword(keyword string) []source.SyntaxError {
...
func (p *Assembler) parseIdentifier() (string, []source.SyntaxError) {
	tok, errs := p.expect(IDENTIFIER)
	//
	if len(errs) > 0 {
		return "", errs
...
func (p *Assembler) string(token lex.Token) string {
	start, end := token.Span.Start(), token.Span.End()
	return string(p.srcfile.Contents()[start:end])
}

// Get the text representing the given token as a string.
func (p *Assembler) number(token lex.Token) big.Int {
	var number big.Int
	//
...
func (p *Assembler) lookahead() lex.Token {
	return p.tokens[p.index]
}

// Expect panics if the next token is not what was expected.
func (p *Assembler) expect(kind uint) (lex.Token, []source.SyntaxError) {
	lookahead := p.lookahead()
	//
...
func (p *Assembler) match(kind uint) bool {
...
func (p *Assembler) syntaxErrors(token lex.Token, msg string) []source.SyntaxError {
...
func (p *Environment) BindLabels(insns []Instruction) {
