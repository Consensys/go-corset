...
func (p *moduleCellFilter) Module(mid uint) ColumnFilter {
...
func (p *columnCellFilter) Column(cid uint) CellFilter {
	var bits bit.Set = bit.Set((*p)[cid])
	//
	if bits.Count() == 0 {
		return nil
	}
