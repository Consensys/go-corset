...
type SortedSet[T uint | int] []T

// NewSortedSet returns an empty sorted set.
func NewSortedSet[T uint | int]() *SortedSet[T] {
	return &SortedSet[T]{}
}
...
// map/reduce function.
func UnionSortedSets[S any, T uint | int](elems []S, fn func(S) *SortedSet[T]) *SortedSet[T] {
	if len(elems) == 0 {
...
func countDuplicates[T uint | int](left []T, right []T) int {
	// Check containment
	i := 0
	j := 0
	n := 0
...
func mergeSorted[T uint | int](target []T, left []T, right []T) {
	i := 0
	j := 0
	k := 0
