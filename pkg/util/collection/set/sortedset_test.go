...
func removeDups(words [N][]uint) [N][]uint {
	var nwords [N][]uint
	// determine height
	n := len(words[0])
	//
	for i := 0; i < n; i++ {
		if i == 0 || !equalWords(i-1, i, words) {
			for j := 0; j < N; j++ {
				nwords[j] = append(nwords[j], words[j][i])
			}
		}
	}
	//
...
func equalWords(lhs, rhs int, words [N][]uint) bool {
	for i := 0; i < N; i++ {
		if words[i][lhs] != words[i][rhs] {
			return false
		}
	}
	//
...
func isZeroWord(index int, words [N][]uint) bool {
	for i := 0; i < N; i++ {
		if words[i][index] != 0 {
			return false
		}
